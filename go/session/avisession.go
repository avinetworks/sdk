...
func (avisess *AviSession) initiateSession() error {
	...
	} else if avisess.isAzureAuth() {
		cred["aztoken"] = avisess.azAuthToken
	} else {
...
// SetAzAuthToken - Use this for NewAviSession option argument for setting azAuthToken
func SetAzAuthToken(azAuthToken string) func(*AviSession) error {
	return func(sess *AviSession) error {
		return sess.setAzAuthToken(azAuthToken)
	}
}

func (avisess *AviSession) setAzAuthToken(azAuthToken string) error {
	avisess.azAuthToken = azAuthToken
	return nil
}
...
func (avisess *AviSession) isAzureAuth() bool {
	return avisess.azAuthToken != ""
}
...
type AviAzureCloudList struct {
	Count   float64
	Results []AviAzureCloud
}
type AviAzureCloud struct {
	//vtype                        string
	//license_tier                 string
	//_last_modified               string
	//enable_vip_static_routes     bool
	Url string
	//ip6Autocfg_enabled           bool
	Tenant_ref string
	Uuid       string
	//dhcp_enabled                 bool
	//prefer_static_routes         bool
	//autoscale_polling_interval   float64
	//license_type                 string
	//mtu                          float64
	//apic_mode                    bool
	//state_based_dns_registration bool
	Azure_configuration AviAzureCfg
	Name                string
}

type AviAzureCfg struct {
	...
	//use_azure_dns         bool
	//location              string
	Network_info []AviAzureNetworkInfo
	//subscription_id       string
	//use_standard_alb      bool
}

type AviAzureNetworkInfo struct {
	//se_network_id      string
	Virtual_network_id string
}

func (avisess *AviSession) GetCloudByVnet(vnet string) (string, error) {
	uri := "api/cloud"
	m := fmt.Sprintf("\nARIF DEBUG uri = %+v", uri)
	res, rerror := avisess.restRequest("GET", uri, nil)
	if rerror != nil {
		return "", rerror
	}
	m += fmt.Sprintf("\nARIF DEBUG res = %+v", res)
	var cloudjson AviAzureCloudList
	err := json.Unmarshal(res, &cloudjson)
	if err != nil {
		return "", err
	}
	for _, cloud := range cloudjson.Results {
		m += fmt.Sprintf("\nARIF DEBUG cloud = %+v", cloud)
		if len(cloud.Azure_configuration.Network_info) > 0 {
			if strings.HasSuffix(cloud.Azure_configuration.Network_info[0].Virtual_network_id, vnet) {
				return cloud.Url, nil
			}
		}
	}
	return "", errors.New(m)
