...
type CCAgentProperties struct {
	...
	// Delay between each async job status poll check. Units(SEC).
	AsyncRetriesDelay int32 `json:"async_retries_delay,omitempty"`

	// Discovery poll target duration; a scale factor of 1+ is computed with the actual discovery (actual/target) and used to tweak slow and fast poll intervals. Units(SEC).
	PollDurationTarget int32 `json:"poll_duration_target,omitempty"`

	// Fast poll interval. Units(SEC).
	PollFastTarget int32 `json:"poll_fast_target,omitempty"`

	// Slow poll interval. Units(SEC).
	...
	// Delay between each vnic status poll check. Units(SEC).
