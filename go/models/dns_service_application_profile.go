...
type DNSServiceApplicationProfile struct {
	...
	// Specifies the ip address prefix length to use in the edns client subnet (ecs) option. When the incoming request does not have any ecs option and the prefix length is specified, we insert an ecs option in the request to upstream servers. Allowed values are 1-32. Field introduced in 17.1.3.
	...
	// Specifies the TTL value (in seconds) for SOA (Start of Authority) (corresponding to a authoritative domain owned by this DNS Virtual Service) record's minimum TTL served by the DNS Virtual Service. Allowed values are 1-86400. Field introduced in 17.2.4. Units(SEC).
	...
	// Specifies the TTL value (in seconds) for records served by DNS Service. Allowed values are 1-86400. Units(SEC).
