...
type SeBootupProperties struct {
	...
	// Controls the use of DPDK poll mode driver      0        Automatically determine based on hypervisor/NIC type    1        Unconditionally use DPDK poll mode driver    ~[0,1]   Don't use DPDK poll mode driver.
	...
	// Determines if DPDK library should be used or not       0        Automatically determine based on hypervisor type    1        Use DPDK unconditionally    ~[0,1]   Don't use DPDK.
