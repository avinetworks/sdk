...
type SeRuntimeCompressionProperties struct {
	...
	// If client RTT is higher than this threshold, enable normal compression on the response. Units(MILLISECONDS).
	MaxLowRtt int32 `json:"max_low_rtt,omitempty"`

	// If client RTT is higher than this threshold, enable aggressive compression on the response. Units(MILLISECONDS).
