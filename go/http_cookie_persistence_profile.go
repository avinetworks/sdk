...
type HTTPCookiePersistenceProfile struct {
	...
	// The length of time after a client's connections have closed before expiring the client's persistence to a server. No value or 'zero' indicates no timeout. Allowed values are 1-14400. Special values are 0- 'No Timeout'. Units(MIN).
