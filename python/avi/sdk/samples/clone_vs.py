...
class AviClone:
    ...
    def clone_generic(self, object_type, old_generic_name, new_generic_name,
        ...
        :param tenant: Tenant for existing pool (if not specfied,
              use user's default tenant)
        :param other_tenant: Tenant for cloned pool (if not specified,
    ...
    def clone_pool(self, old_pool_name, new_pool_name, tenant = None,
        ...
        :param force_unique_name: Resolve destination name conflicts by
                                    appending an index number
        :return: tuple - json representation of the cloned pool object, list of
            ...
            if 'members' in pg_obj:
                ...
                for member in pg_obj['members']:
                    if 'pool_ref' in member:
                        ...
                        p_obj, p_created_objs = self.clone_pool(p_path,
                                    new_pool_group_name + '-pool-' + str(count),
    ...
    def clone_vs(self, old_vs_name, new_vs_name, enable_vs = False,
        ...
        try:
            ...
            if 'pool_ref' in v_obj:
                ...
                p_obj, p_created_objs = self.clone_pool(p_path, new_vs_name +
    ...
    def clone_refs_to_tenant(self, parent_obj, refs, t_obj, ot_obj):
        ...
        # Process the list of child objects, refs, of the parent_obj. If the child object
        # is not a global object, then either update the reference to point to an object
        # of the same name in the target tenant or clone the object to the target
        # tenant.
                ...
                for i in range(len(child_objs)):
                    ...
                    if r_obj.status_code == 404:
                        ...
                        old_r_obj = self.api.get(r_obj_path,
                                                 tenant_uuid = t_obj['uuid']).json()
                        new_r_obj = self.api.get_object_by_name(r_obj_type,
                                                 old_r_obj['name'],
                                                 tenant_uuid = ot_obj['uuid'])

                        if new_r_obj is None:
                            # Otherwise clone the object to the target tenant context
                            new_r_obj = self.clone_generic(r_obj_type, r_obj_path,
                                                 old_r_obj['name'], t_obj['name'],
                                                 ot_obj['name'], True)
            ...
            if args.obj_type == 'vs':
                ...
                for new_pool_name in args.new_pool_names.split(','):
                    spprint('Trying to clone pool %s%s to %s%s%s...' %
                            ...
                            args.totenant else '', ' in cloud '+args.tocloud if
                            args.tocloud else ''), end = '', flush = True)
                    new_pool, created_objs = cl.clone_pool(args.pool_name,
                                new_pool_name, args.tenant, args.totenant,
                                args.tocloud, False)
                    print('OK!')
                    print()
